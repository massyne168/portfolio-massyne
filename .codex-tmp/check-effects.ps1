...
$r=Call-CDP 'Runtime.evaluate' @{expression="JSON.stringify({client:document.documentElement.clientWidth,scroll:document.documentElement.scrollWidth,separators:document.querySelectorAll('.chapter-break').length,hiddenArtwork:[...document.querySelectorAll('.project-image img')].some(i=>getComputedStyle(i).opacity==='0'),driftEligible:finePointer.matches,driftObserved:visibleDrift.size})";returnByValue=$true}
$r.result.value
Call-CDP 'Emulation.setEmulatedMedia' @{features=@(@{name='prefers-reduced-motion';value='reduce'})} | Out-Null
Start-Sleep -Milliseconds 150
$r=Call-CDP 'Runtime.evaluate' @{expression="JSON.stringify({reduced:motionPreference.matches,animations:document.getAnimations().length,driftObserved:visibleDrift.size,driftStyles:driftTargets.some(i=>i.style.getPropertyValue('--image-drift')),scrollBehavior:getComputedStyle(document.documentElement).scrollBehavior})";returnByValue=$true}
$r.result.value
